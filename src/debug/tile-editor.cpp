...
Tile_Editor::Tile_Editor(SDL_Renderer* renderer) : Renderable(renderer)
{
	this->map = nullptr;
	cursor_pos = Ivec(0, 0);
	clear();
...
void Tile_Editor::render()
{
	...
	{
		// draw cursor
		SDL_Rect rect = { cursor_pos.x, cursor_pos.y, tile_size.x,
						  tile_size.y };
		...
		rect = { (cursor_pos.x + tile_size.x) * 2, cursor_pos.y * 2, tex_size.x,
				 tex_size.y };
		...
		rect = { cursor_pos.x + tile_size.x + sel_type.x * tile_size.x / 2,
				 cursor_pos.y + sel_type.y * tile_size.y / 2, tile_size.x / 2,
				 tile_size.y / 2 };
		SDL_RenderDrawRect(renderer, &rect);
			...
			switch(event.key.keysym.sym)
			{
				case SDLK_h:
				{
					if(keys[SDL_SCANCODE_LSHIFT])
					{
						sel_type.x--;
						Ivec tex_size;
						SDL_QueryTexture(full_texture, NULL, NULL, &tex_size.x,
										 &tex_size.y);
						if(sel_type.x < 0)
						{
				...
				case SDLK_n:
				{
					if(keys[SDL_SCANCODE_LCTRL])
					{
						...
						std::ifstream is(original_path,
										 std::ios::in | std::ios::binary);
						std::ofstream os(original_path + "_copy",
...
Tile* Tile_Editor::hovered_tile()
{
	...
	Tile* old_tile = map->get_tiles()[index_to_replace.x][index_to_replace.y];
	return old_tile;
}
